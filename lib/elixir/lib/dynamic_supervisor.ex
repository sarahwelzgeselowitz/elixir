defmodule DynamicSupervisor do
  ...
  def start_link(options) when is_list(options) do
    keys = [:strategy, :max_seconds, :max_restarts, :max_children]
  ...
  def init(options) when is_list(options) do
    ...
    {:ok, %{strategy: strategy, intensity: intensity, period: period, max_children: max_children}}
  end
  ...
  defp init(state, flags) do
    ...
    with :ok <- validate_strategy(strategy),
         ...
         :ok <- validate_dynamic(max_children) do
      {:ok, %{
        state
        | strategy: strategy,
